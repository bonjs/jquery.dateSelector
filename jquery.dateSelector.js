...
$.fn.dateSelector = function() {
	...
	var collapseModule = function() {
		var html = [
			'<div class="dateSelector-collapse hide">',
				'<div class="left">',
	...
	var yearModule = function() {
		...
		collapse.on('click', 'div.year li', function(a, b) {	// 点击年时, 年的样式更改
			collapse.find('div.year li').removeClass('dateSelector-collapse-li-select');
			$(this).addClass('dateSelector-collapse-li-select');
		...
		return {
			...
			step: step,
			getValue: function() {
				return yearDiv.find('ul li.dateSelector-collapse-li-select').html();
			},
			setValue: function(year) {
				...
				years.each(function(i, it) {
					if($(it).html() == year) {
						$(it).addClass('dateSelector-collapse-li-select');
					} else {
						$(it).removeClass('dateSelector-collapse-li-select');
					}
				});
			}
		};
	...
	var monthModule = function() {
		...
		collapse.on('click', 'div.month li', function(a, b) {
			collapse.find('div.month li').removeClass('dateSelector-collapse-li-select');
			$(this).addClass('dateSelector-collapse-li-select');
		...
		return {
			showMonth: function() {		//根据是否是今年来决定是否显示所有的月份
				...
				collapse.find('.month li').removeClass('dateSelector-collapse-li-select');
			},
			...
			getValue: function() {
				return months.find('li.dateSelector-collapse-li-select').attr('month');
			}
		};
	}();
	...
	if(me.val() == '') {
		...
		monthModule.setValue(date.getMonth() + 1);
	}
